...
module.exports = function(req, res, next) {
  ...
  async.waterfall([
      // Check redirect uri
      function(cb) {
        // removing validation for redirect_uri. Standard is redirectUri is optional
      ...
      function(cb) {
        if (!req.oauth2.model.client.getRedirectUri(client))
          ...
          req.oauth2.logger.debug('RedirectUri not provided by call using client');
          cb();
    ...
    function(err) {
			var _redirectUri = null;
			if(client) {
				_redirectUri = req.oauth2.model.client.getRedirectUri(client);
			}else{
				_redirectUri = redirectUri;
			}
      if (err) response.error(req, res, err, _redirectUri);
      else {
        if (req.method == 'GET')
          req.oauth2.decision(req, res, client, scope, user, _redirectUri);
        else if (grantType == 'authorization_code')
          authorization.code(req, res, client, scope, user, _redirectUri);
        else if (grantType == 'implicit')
          authorization.implicit(req, res, client, scope, user, _redirectUri);
        else
          response.error(req, res, new error.invalidRequest('Wrong request method'), _redirectUri);
      }
    });
};
